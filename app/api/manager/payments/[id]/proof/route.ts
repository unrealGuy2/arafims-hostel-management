...
export async function GET(
  ...
  const { data: payment, error } = await supabase
    .from("payments")
    .select("payment_proof_path, reservation:reservation_id(room:room_id(hostel_id))")
    .eq("id", id)
    .single();
  ...
  const { data: signedUrl, error: signedUrlError } = await supabase.storage
    .from("payment_proofs")
    .createSignedUrl(payment.payment_proof_path, 300);

  if (signedUrlError || !signedUrl?.signedUrl) {
    return NextResponse.json({ message: "Unable to open payment proof" }, { status: 500 });
  }
