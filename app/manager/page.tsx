...
export default async function ManagerPage({
  ...
  const { data: reservations } = roomIds.length
    ? await supabase
        .from("reservations")
        .select(
          "id, status, room_price, created_at, decision_reason, student_profiles(full_name, email, matric_number, gender, level, department, faculty, previous_hostel), rooms(room_number, room_type, capacity)"
        )
        .in("room_id", roomIds)
